...
public:
	 ...
	 */
	explicit pca(const long& num_vars);
	/**
	 ...
	 * @throws std::invalid_argument if num_vars is smaller than two
	 */
	void set_num_variables(const long& num_vars);
	/**
	 ...
	 * @return The record
	 */
	std::vector<double> get_record(const long& record_index) const;
	/**
	 ...
	 */
	void set_do_normalize(const bool& do_normalize);
	/**
	 ...
	 */
	void set_do_bootstrap(const bool& do_bootstrap, const long& number=30, const long& seed=1);
	/**
	 ...
	 */
	void set_num_retained(const long& num_retained);
	/**
	 ...
	 */
	double get_eigenvalue(const long& eigen_index) const;
	/**
	 ...
	 * @throws std:range_error if eigen_index is out of range
	 */
	std::vector<double> get_eigenvalue_boot(const long& eigen_index) const;
	/**
	 ...
	 * @throws std:range_error if eigen_index is out of range
	 */
	std::vector<double> get_eigenvector(const long& eigen_index) const;
	/**
	 ...
	 * @throws std:range_error if eigen_index is out of range
	 */
	std::vector<double> get_principal(const long& eigen_index) const;
	/**
...
/**
 * @brief Checks if two iterators are approx. equal relative to an epsilon
 * @param iter1 Some iterator
 * @param iter2 Another iterator
 * @param eps The epsilon
 * @returns Whether the two iterators are approx. equal
 */
template<typename T, typename U, typename V>
bool is_approx_equal_iter(const T& iter1, const U& iter2, const V& eps) {
	if (iter1.size()==iter2.size()) {
		bool equal = true;
		for (size_t i=0; i<iter1.size(); ++i) {
			equal = is_approx_equal(iter1[i], iter2[i], eps);
			if (!equal) break;
		}
		return equal;
	} else {
...
bool is_equal_iter(const T& iter1, const U& iter2) {
	if (iter1.size()==iter2.size()) {
		bool equal = true;
		for (size_t i=0; i<iter1.size(); ++i) {
			equal = iter1[i]==iter2[i];
			if (!equal) break;
		}
		return equal;
	} else {
...
//	}
//	template<typename T, typename... Args>
//	friend std::string join(const T& arg, const Args&... args);
//};
/**
 * @brief Joins an arbitrary number of input arguments to a single string
 * @param arg An argument
 * @param args An arbitrary number of arguments (can be also omitted)
 ...
 */
template<typename T>
void write_property(std::ofstream& file, const std::string& key, const T& value) {
 ...
 */
template<typename T>
void read_property(std::ifstream& file, const std::string& key, T& value) {
	std::string tmp;
	bool found = false;
	while (file.good()) {
		file >> tmp;
	...
	if (!found)
	{
		cout << "No such key available: " << key << endl;
	}
	file.seekg(0);
}
