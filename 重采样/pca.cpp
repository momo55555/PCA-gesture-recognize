...
stats::pca::pca(const long& num_vars)
	: num_vars_(num_vars),
	  num_records_(0),
	  record_buffer_(1000),
	  solver_("dc"),
...
stats::pca& stats::pca::operator=(const pca& other) {
	if (this != &other) {
		num_vars_ = other.num_vars_;
		num_records_ = other.num_records_;
		record_buffer_ = other.record_buffer_;
...
bool stats::pca::operator==(const pca& other) {
	...
	if (num_vars_ == other.num_vars_ &&
		...
		bootstrap_seed_ == other.bootstrap_seed_ &&
		num_retained_ == other.num_retained_ &&
		utils::is_approx_equal_iter(eigval_, other.eigval_, eps) &&
		utils::is_approx_equal_iter(eigvec_, other.eigvec_, eps) &&
		utils::is_approx_equal_iter(princomp_, other.princomp_, eps) &&
		utils::is_approx_equal_iter(energy_, other.energy_, eps) &&
		utils::is_approx_equal_iter(mean_, other.mean_, eps) &&
		utils::is_approx_equal_iter(sigma_, other.sigma_, eps) &&
		utils::is_approx_equal_iter(eigval_boot_, other.eigval_boot_, eps) &&
		utils::is_approx_equal_iter(energy_boot_, other.energy_boot_, eps) &&
		utils::is_approx_equal_iter(proj_eigvec_, other.proj_eigvec_, eps))
		return true;
	else
		return false;
}
...
void stats::pca::resize_data_if_needed_() {
...
void stats::pca::set_num_variables(const long& num_vars) {
	num_vars_ = num_vars;
	assert_num_vars_();
	num_retained_ = num_vars_;
	data_.resize(record_buffer_, num_vars_);
...
void stats::pca::add_record(const std::vector<double>& record) {
	assert_num_vars_();

	if (num_vars_ != long(record.size()))
		cout << "Record has the wrong size: " << record.size();
...
std::vector<double> stats::pca::get_record(const long& record_index) const {
	return std::move(utils::extract_row_vector(data_, record_index));
}

void stats::pca::set_do_normalize(const bool& do_normalize) {
	do_normalize_ = do_normalize;
}

void stats::pca::set_do_bootstrap(const bool& do_bootstrap, const long& number, const long& seed) {
	if (number < 10)
...
void stats::pca::set_solver(const std::string& solver) {
	if (solver!="standard" && solver!="dc")
		cout << "No such solver available: " << solver;
	solver_ = solver;
}

void stats::pca::solve() {
	assert_num_vars_();
...
void stats::pca::bootstrap_eigenvalues_() {
...
void stats::pca::set_num_retained(const long& num_retained) {
	if (num_retained<=0 || num_retained>num_vars_)
		cout << "Value out of range: " << num_retained;

	num_retained_ = num_retained;
	proj_eigvec_ = eigvec_.submat(0, 0, eigvec_.n_rows-1, num_retained_-1);
}

std::vector<double> stats::pca::to_principal_space(const std::vector<double>& data) const {
...
std::vector<double> stats::pca::to_variable_space(const std::vector<double>& data) const {
...
double stats::pca::get_energy() const {
	return energy_(0);
}

std::vector<double> stats::pca::get_energy_boot() const {
	return std::move(utils::extract_column_vector(energy_boot_, 0));
}

double stats::pca::get_eigenvalue(const long& eigen_index) const {
	if (eigen_index >= num_vars_)
		cout << "Index out of range: " << eigen_index;
	return eigval_(eigen_index);
}

std::vector<double> stats::pca::get_eigenvalues() const {
	return std::move(utils::extract_column_vector(eigval_, 0));
}

std::vector<double> stats::pca::get_eigenvalue_boot(const long& eigen_index) const {
	return std::move(utils::extract_column_vector(eigval_boot_, eigen_index));
}

std::vector<double> stats::pca::get_eigenvector(const long& eigen_index) const {
	return std::move(utils::extract_column_vector(eigvec_, eigen_index));
}

std::vector<double> stats::pca::get_principal(const long& eigen_index) const {
	return std::move(utils::extract_column_vector(princomp_, eigen_index));
}

double stats::pca::check_eigenvectors_orthogonal() const {
	return std::abs(arma::det(eigvec_));
}

double stats::pca::check_projection_accurate() const {
...
bool stats::pca::get_do_normalize() const {
	return do_normalize_;
}

bool stats::pca::get_do_bootstrap() const {
	return do_bootstrap_;
}

long stats::pca::get_num_bootstraps() const {
	return num_bootstraps_;
}

long stats::pca::get_bootstrap_seed() const {
	return bootstrap_seed_;
}

std::string stats::pca::get_solver() const {
	return solver_;
}

std::vector<double> stats::pca::get_mean_values() const {
	return std::move(utils::extract_column_vector(mean_, 0));
}

std::vector<double> stats::pca::get_sigma_values() const {
	return std::move(utils::extract_column_vector(sigma_, 0));
}

long stats::pca::get_num_variables() const {
	return num_vars_;
}

long stats::pca::get_num_records() const {
	return num_records_;
}

long stats::pca::get_num_retained() const {
	return num_retained_;
}

void stats::pca::save(const std::string& basename) const {
...
void stats::pca::load(const std::string& basename) {
	const std::string filename = basename + ".pca";
