...
arma::Mat<double> stats::utils::make_covariance_matrix(const arma::Mat<double>& data) {
	return std::move( (data.t()*data) * (1./(data.n_rows-1)) );
}

arma::Mat<double> stats::utils::make_shuffled_matrix(const arma::Mat<double>& data) {
...
arma::Col<double> stats::utils::compute_column_means(const arma::Mat<double>& data) {
	const long n_cols = data.n_cols;
...
void stats::utils::remove_column_means(arma::Mat<double>& data, const arma::Col<double>& means) {
...
arma::Col<double> stats::utils::compute_column_rms(const arma::Mat<double>& data) {
	const long n_cols = data.n_cols;
...
void stats::utils::normalize_by_column(arma::Mat<double>& data, const arma::Col<double>& rms) {
...
void stats::utils::enforce_positive_sign_by_column(arma::Mat<double>& data) {
	for (long i=0; i<long(data.n_cols); ++i) {
...
std::vector<double> stats::utils::extract_column_vector(const arma::Mat<double>& data, const long& index) {
...
std::vector<double> stats::utils::extract_row_vector(const arma::Mat<double>& data, const long& index) {
...
void stats::utils::assert_file_good(const bool& is_file_good, const std::string& filename) {
	if (!is_file_good)
	{
...
double stats::utils::get_mean(const std::vector<double>& iter) {
	const double init = 0;
	return std::accumulate(iter.begin(), iter.end(), init) / iter.size();
}

double stats::utils::get_sigma(const std::vector<double>& iter) {
